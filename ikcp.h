...
// queue operation                     
//---------------------------------------------------------------------
/// node放在head后面，head->node
#define IQUEUE_ADD(node, head) ( \
	(node)->prev = (head), (node)->next = (head)->next, \
	(head)->next->prev = (node), (head)->next = (node))

/// head放在node后面，node->head
...
/// 删除entry, pre->entry->next  ====>>>>  pre->next
...
struct IKCPSEG
{
    ...
    IUINT32 fastack;        // ack跳过的次数，用于快速重传。在每帧调用input时，没有该seg的ack包，则fastack++。
...
struct IKCPCB
{
    ...
    //rx_rttval：ack接收rtt浮动值，rx_srtt：ack接收rtt平滑值(smoothed)，rx_rto：由ack接收延迟计算出来的复原时间，rx_minrto：最小复原时间,rto最少为rx_srtt + rx_minrto
    IINT32 rx_rttval, rx_srtt, rx_rto, rx_minrto;
    //sen_wnd：发送窗口大小，rcv_wnd：接收窗口大小，rmt_wnd：远端可用窗口大小，cwnd：拥塞窗口大小，probe  探查变量，IKCP_ASK_TELL表示告知远端窗口大小。IKCP_ASK_SEND表示请求远端告知窗口大小。
    IUINT32 snd_wnd, rcv_wnd, rmt_wnd, cwnd, probe;
    //currunt：当前的时间戳，interval：内部flush刷新间隔，ts_flush：下次flush刷新时间戳, xmit：重发消息的次数，只单纯计数，没看到使用的地方
    IUINT32 current, interval, ts_flush, xmit;
	//nrcv_buf: snd_buf的长度   nsnd_buf: rcv_buf的长度
    IUINT32 nrcv_buf, nsnd_buf;
	// nrcv_que: rcv_queue的长度  nsnd_que: snd_queue的长度
    IUINT32 nrcv_que, nsnd_que;
    //nodelay：是否启动无延迟模式，延迟返回ack包的目的是，在延迟时间内积累更多的ack包一次性发送，节省流量；启动nodelay则响应更快
	// update：是否调用过update函数的标识(kcp需要上层通过不断的ikcp_update和ikcp_check来驱动kcp的收发过程)，只有初始化的时候是false，调用后就永远变成了true
    IUINT32 nodelay, updated;
    //ts_probe：下次探查窗口的时间戳，probe_wait：探查窗口需要等待的时间
    IUINT32 ts_probe, probe_wait;
    //dead_link：最大重传次数，incr：可发送的最大数据量，一般情况下，incr=cwnd*mss，当cwnd超过ssthresh时，incr就不是mss整数倍了，累加一个小于mss的数值[8]
    ...
    IUINT32 *acklist;               //待发送的ack的列表 // 里面的元素是sn1,timestamp1,sn2,timestamp2 ……
    ...
    int nocwnd, stream;             //nocwnd：取消拥塞控制，取消后cwnd和ssthresh就不起作用了  stream：是否采用流传输模式
    int logmask;
